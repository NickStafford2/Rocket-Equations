...
lensflare.addElement(
  new LensflareElement(textureFlare2, 64, 0.4, new THREE.Color(0.8, 0.8, 1)),
);
lensflare.addElement(
  new LensflareElement(textureFlare2, 32, 0.6, new THREE.Color(1, 0.8, 0.6)),
);
sun.add(lensflare);
...
function keplerToCartesian(orb, epochJD = 2460000) {
  ...
  const x =
    x_orb *
      (Math.cos(w) * Math.cos(om) - Math.sin(w) * Math.sin(om) * Math.cos(i)) -
    y_orb *
      (Math.sin(w) * Math.cos(om) + Math.cos(w) * Math.sin(om) * Math.cos(i));
  const y =
    x_orb *
      (Math.cos(w) * Math.sin(om) + Math.sin(w) * Math.cos(om) * Math.cos(i)) +
    y_orb *
      (Math.cos(w) * Math.cos(om) * Math.cos(i) - Math.sin(w) * Math.sin(om));
  const z =
    x_orb * Math.sin(w) * Math.sin(i) + y_orb * Math.cos(w) * Math.sin(i);
  return { x, y, z };
}
...
function createEnhancedAsteroidBelt() {
  ...
  for (let i = 0; i < innerCount; i++) {
    ...
    const radius =
      innerInnerRadius + Math.random() * (innerOuterRadius - innerInnerRadius);
  ...
  for (let i = 0; i < middleCount; i++) {
    ...
    const radius =
      middleInnerRadius +
      Math.random() * (middleOuterRadius - middleInnerRadius);
  ...
  for (let i = 0; i < outerCount; i++) {
    ...
    const radius =
      outerInnerRadius + Math.random() * (outerOuterRadius - outerInnerRadius);
...
const celestialBodies = [
  ...
  },
  {
    ...
    moons: [
      ...
      },
      {
        ...
        dist: 4.2,
        speed: 0.28,
        color: new THREE.Color(0.53, 0.81, 0.92),
      ...
      },
      {
        ...
        dist: 5.1,
        speed: 0.14,
        color: new THREE.Color(0.55, 0.49, 0.42),
    ...
    moons: [
      {
        ...
        dist: 2.2,
        speed: 0.02,
        color: new THREE.Color(0.87, 0.87, 0.87),
    ...
    moons: [
      {
        ...
        dist: 1.6,
        speed: 0.083,
        color: new THREE.Color(0.33, 0.33, 0.33),
        info: "Moon of Quaoar.",
        initialAngle: 1.3,
      },
    ],
    ...
    moons: [
      {
        ...
        dist: 1.9,
        speed: 0.1,
        color: new THREE.Color(0.27, 0.27, 0.27),
  ...
  },
  {
    ...
    speed: 0.0000019,
    initialAngle: 3.7,
    color: new THREE.Color(0.45, 0.15, 0.1),
    roughness: 1.0,
    metalness: 0.0,
    type: "dwarf",
...
function animate() {
  ...
  if (frameCount % 60 === 0) {
    console.log(
      `Animation running. Frame: ${frameCount}, Paused: ${isPaused}, Speed: ${animationSpeed}`,
    );
  }
  frameCount++;
  ...
  if (!isPaused) {
    ...
    planetMeshes.forEach((p) => {
      ...
      if (p.orbit && p.orbit.userData && p.orbit.userData.pulseSpeed) {
        try {
          ...
          const pulse =
            Math.sin(
              time * p.orbit.userData.pulseSpeed + p.orbit.userData.pulsePhase,
            ) *
              0.3 +
            0.7;

          if (p.orbit.material.emissiveIntensity !== undefined) {
            p.orbit.material.emissiveIntensity =
              p.orbit.userData.originalEmissive * pulse;
          }

          if (!p.orbit.userData.originalOpacity) {
            p.orbit.userData.originalOpacity = p.orbit.material.opacity;
          }
          p.orbit.material.opacity =
            p.orbit.userData.originalOpacity * (0.8 + pulse * 0.2);
        } catch (error) {
      ...
      if (p.moons && p.moons.length > 0) {
        p.moons.forEach((moon) => {
          ...
          moon.mesh.rotation.y +=
            0.02 * realTimeMultiplier * (eclipseTourActive ? 0.6 : 1.0);
        });
      }
    });
    ...
    Object.values(asteroidBelts).forEach((belt) => {
      belt.forEach((asteroid) => {
        asteroid.mesh.rotation.x +=
          asteroid.rotationSpeed.x * realTimeMultiplier;
        asteroid.mesh.rotation.y +=
          asteroid.rotationSpeed.y * realTimeMultiplier;
        asteroid.mesh.rotation.z +=
          asteroid.rotationSpeed.z * realTimeMultiplier;

        asteroid.angle +=
          asteroid.orbitSpeed *
          realTimeMultiplier *
          (eclipseTourActive ? 0.06 : 1.0);
    ...
    distantStars.rotation.y +=
      0.0001 * realTimeMultiplier * (eclipseTourActive ? 0.06 : 1.0);
  }
  ...
  try {
    ...
    console.error(
      "Composer rendering failed, falling back to direct rendering:",
      error,
    );
...
if (labelToggle) {
  labelToggle.addEventListener("click", () => {
    ...
    labelToggle.textContent = showPlanetLabels
      ? "Hide Planet Names"
      : "Show Planet Names";
...
if (moonLabelToggle) {
  moonLabelToggle.addEventListener("click", () => {
    ...
    moonLabelToggle.textContent = showMoonLabels
      ? "Hide Moon Names"
      : "Show Moon Names";
...
if (allAsteroidsBtn) {
  allAsteroidsBtn.addEventListener("click", () => {
    const allVisible =
      !asteroidBelts.inner[0]?.mesh.visible ||
    ...
    if (mainAsteroidsBtn)
      mainAsteroidsBtn.classList.toggle("active", allVisible);
  ...
  if (body.moons && body.moons.length > 0) {
    ...
    body.moons.forEach((moon, moonIndex) => {
      ...
      const orbitalPeriodDays =
        moon.speed > 0 ? ((2 * Math.PI) / moon.speed).toFixed(1) : "Unknown";
...
function startEclipseTour() {
  ...
  eclipseCameraPositions = [
    ...
    {
      position: new THREE.Vector3(15, 8, 5),
      target: new THREE.Vector3(15, 0, 0),
    },
  ];
  ...
  if (infoPanel) {
    ...
    const emojiElement = infoPanel.querySelector(
      'div[style*="font-size:40px"]',
    );

    if (eclipseType === "solar") {
      if (titleElement)
        titleElement.textContent = "Solar Eclipse: Cinematic Tour";
      if (emojiElement) emojiElement.textContent = "🌒";
    } else {
      if (titleElement)
        titleElement.textContent = "Lunar Eclipse: Cinematic Tour";
      if (emojiElement) emojiElement.textContent = "🌕";
    }
  }
  ...
  const uiEls = document.querySelectorAll(
    ".controls, .celestial-panel, .info, #planetInfoCard, #showUIBtn",
  );
  uiEls.forEach((el) => {
    if (el) {
...
function updateEclipsePhaseInfo() {
  ...
  const phaseInfo =
    eclipseType === "solar" ? solarEclipseInfo : lunarEclipseInfo;
...
function stopEclipseTour() {
  ...
  if (
    originalEclipsePositions.moon !== undefined &&
    planetMeshes[2].moons &&
    planetMeshes[2].moons[0]
  ) {
...
document
  .getElementById("closePlanetInfo")
  .addEventListener("click", hidePlanetInfoCard);
...
document.getElementById("followPlanetBtn").addEventListener("click", () => {
  if (currentPlanetIndex !== null) {
    if (
      followingTarget === planetMeshes[currentPlanetIndex] &&
      followingType === "planet"
    ) {
      stopFollowingPlanet();
    } else {
...
document.addEventListener("DOMContentLoaded", () => {
  const originalPlanetItemHandler = (planetItem) => {
    ...
    planetItem.onclick = function (event) {
      ...
      const planetIndex = celestialBodies.findIndex(
  ...
  switch (event.key.toLowerCase()) {
    ...
    case "a": // A to cycle through asteroid belt types
      ...
      for (let i = 0; i < asteroidButtons.length; i++) {
        if (
          asteroidButtons[i] &&
          asteroidButtons[i].classList.contains("active")
        ) {
    ...
    case "b": // B to toggle bloom mode (Auto/Manual)
      ...
      const bloomModeBtn = document.getElementById("bloomModeBtn");
      if (bloomModeBtn) {
        bloomModeBtn.textContent = isBloomManual
          ? "Auto Bloom"
          : "Manual Bloom";
        bloomModeBtn.classList.toggle("active", isBloomManual);
      }
      ...
      if (!isBloomManual) {
        console.log(
          "🌟 Switched to automatic bloom mode (dynamic with distance)",
        );
      } else {
